...
a0 = 12.5.*2.*Ts/T0; # o coeficiente a0
ak =@(k) 12.5.*(2./(k.*pi)).*sin(k.*pi./2);  # o coeficientes como função de k
bk =@(k) 0;
...
figure(4);
subplot(2,1,1);
stem([0 ks], [c0 ck(ks)], 'linewidth', 2, 'MarkerSize',2);
grid on;
xlabel('k');
l21 = legend('c_k(k)');
set(l21, 'fontsize', 14);
...
subplot(2,1,2);
stem([0 ks], [0 thk(ks)], 'linewidth', 2, 'MarkerSize',2);
grid on;
xlabel('k');
